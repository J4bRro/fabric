...
func CreateAndJoinChannel(sdk *fabsdk.FabricSDK, info *InitInfo) error {
	...
	_, err = info.OrgResMgmt.SaveChannel(channelReq, resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithOrdererEndpoint(info.OrdererOrgName))
	if err != nil {
...
func InstallAndInstantiateCC(sdk *fabsdk.FabricSDK, info *InitInfo) error {
	...
	instantiateCCReq := resmgmt.InstantiateCCRequest{Name: info.ChaincodeID, Path: info.ChaincodePath, Version: info.ChaincodeVersion, Args: [][]byte{[]byte("init")}, Policy: ccPolicy}
